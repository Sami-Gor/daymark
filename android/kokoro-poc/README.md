...
KokoroTtsTestActivity (native UI)
  -> KokoroLocalTtsEngine  (implements LocalTtsEngine: initialize/speak/stop/isSpeaking/shutdown)
       ...
       -> nativeSynth(text) -> float[24000 Hz mono PCM]
       -> nativeRelease()
...
| Memory during synthesis | heapUsedKb ~3,000–4,300 / PSS ~455–559 MB (grows per synthesis; not optimized) |
...
| Model | 92,361,116 B |
| Voice | 522,240 B |
| `libkokoro_jni.so` (Rust + static ONNX Runtime) | 61,188,576 B |
| `libc++_shared.so` (NDK runtime) | 9,290,184 B |
| Debug APK | 168,410,891 B |
| Release APK | 164,402,301 B |
| Release AAB | 84,137,751 B |
| Daymark baseline AAB (main, TWA only) | 1,180,563 B |
| AAB increase | **+82,957,188 B (~79.1 MiB)** |
...
- Measurements are emulator-only; latency on real phones is expected to be
  substantially lower.
- PSS grows with each synthesis (no model/session reuse tuning attempted);
